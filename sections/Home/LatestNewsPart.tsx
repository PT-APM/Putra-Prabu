...
export default function LatestNewsPart({latestNews, locale} : LatestNewsPartProps){
    ...
    return  <section className="bg-gradient-to-b from-surface-container-lowest via-surface to-surface-container-lowest py-section-padding px-gutter border-t border-outline-variant/30 relative">
        ...
        <div className="max-w-container-max mx-auto relative z-10">
          ...
          >
            <h2 className="font-headline-lg text-headline-lg text-primary">{dict.home.latestNews.heading}</h2>
            <Link href={localeHref(locale, '/news')} className="font-label-sm text-label-sm text-primary hover:text-primary-container transition-colors flex items-center gap-1 bg-surface-variant/50 px-4 py-2 rounded-full shadow-sm hover:shadow-soft">
              {dict.home.latestNews.viewAll} <span className="material-symbols-outlined text-sm">arrow_forward</span>
            </Link>
          </motion.div>
          ...
          >
            {latestNews.map((news) => (
              <motion.div variants={fadeIn} key={news.id} className="h-full">
                <Link href={localeHref(locale, `/news/${news.id}`)} className="group bg-surface-container-lowest border border-white/60 rounded-3xl overflow-hidden shadow-soft hover:shadow-soft-hover hover:-translate-y-1 transition-all duration-300 block h-full flex flex-col">
